...
![System Architecture](docs/images/arch.png)
...
Unlike a standard user-based lookup, this system operates as a **Session-Based Recommender**. This allows it to serve anonymous users without a login.

1.  **Query Construction (The "Fake User" Vector):**
    * The client sends a list of "Seed Movies" (IDs).
    * Lambda looks up the pre-computed **Item Vectors** for these seeds (generated by the Two-Tower model).
    * We apply **Average Pooling** to these vectors to create a temporary `Query Vector` ($u$) representing the user's current session intent.

2.  **Retrieval (SageMaker):**
    * The `Query Vector` is sent to SageMaker.
    * We perform a **Dot Product Scan** against the cached Item Index (all ~27k movies).
    * Returns the Top-200 candidates by cosine similarity.

3.  **Re-ranking (Lambda):**
    * **SVD Re-rank:** We refine the list using a separate Latent Factor model (TruncatedSVD) trained on user-centered ratings.
    * **MMR Diversity:** We apply Maximal Marginal Relevance to penalize items that are too similar to what's already in the list, ensuring the user doesn't just get 5 clones of the same movie.
